      ...
      {statistics && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px', marginBottom: '32px' }}>
          <div style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: '12px', padding: '24px' }}>
            ...
            <div style={{ fontSize: '36px', fontWeight: '700', color: '#fff' }}>{statistics.totalShifts}</div>
          </div>
          <div style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: '12px', padding: '24px' }}>
            <div style={{ color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '8px' }}>Tổng nhân viên</div>
            <div style={{ fontSize: '36px', fontWeight: '700', color: '#fff' }}>{statistics.totalEmployees}</div>
          </div>
          <div style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: '12px', padding: '24px' }}>
            <div style={{ color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '8px' }}>Tổng phân công</div>
            <div style={{ fontSize: '36px', fontWeight: '700', color: '#fff' }}>{statistics.totalAssignments}</div>
          </div>
        </div>
      )}
            ...
            {Object.values(statistics.assignmentsByShift).map((shiftData, index) => (
              <div key={index} style={{ background: 'rgba(0, 0, 0, 0.15)', border: '1px solid var(--border-glass)', borderRadius: '8px', padding: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '16px', color: '#fff', margin: 0 }}>
                    {shiftData.shiftName}
                  </h3>
                  {shiftData.utilization && (
                    <span style={{ color: shiftData.utilization > 80 ? '#22c55e' : shiftData.utilization > 50 ? '#eab308' : '#ef4444', fontSize: '13px', fontWeight: '600' }}>
                      {shiftData.utilization}%
                    </span>
                  )}
                </div>
                  ...
                  <div style={{ width: '100%', height: '8px', background: 'rgba(255, 255, 255, 0.1)', borderRadius: '4px', overflow: 'hidden' }}>
                    <div
                      style={{
                        ...
                        height: '100%',
                        background: shiftData.utilization > 80 ? '#22c55e' : shiftData.utilization > 50 ? '#eab308' : '#ef4444',
                        borderRadius: '4px',
              ...
              <tbody>
                {Object.values(statistics.assignmentsByEmployee).map((empData, index) => (
                  <tr key={index} style={{ borderBottom: '1px solid var(--border-glass)' }}>
                    <td style={{ padding: '16px', color: '#fff', fontSize: '14px', fontWeight: '500' }}>
                      {empData.userName}
                    </td>
