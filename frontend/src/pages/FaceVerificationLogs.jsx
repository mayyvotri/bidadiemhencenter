...
export default function FaceVerificationLogs() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      const data = await faceRecognitionApi.getVerificationLogs(params);
      if (data.success) {
        setLogs(data.data);
      }
    } catch (err) {
      setError(err.message || 'Không thể tải logs xác thực');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ padding: '32px' }}>
      <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: '32px', color: '#fff', marginBottom: '8px' }}>
        Logs Xác Thực Khuôn Mặt
      </h1>
      <p style={{ color: 'var(--text-secondary)', marginBottom: '32px' }}>
        Xem lịch sử xác thực khuôn mặt của nhân viên.
      </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'rgba(0, 0, 0, 0.2)', borderBottom: '1px solid var(--border-glass)' }}>
              ...
              <th style={{ padding: '16px', textAlign: 'left', color: 'var(--text-secondary)', fontSize: '13px', fontWeight: '600' }}>Độ chính xác</th>
          ...
          <tbody>
            {logs.length === 0 ? (
              ...
              <tr key={log._id} style={{ borderBottom: '1px solid var(--border-glass)' }}>
                ...
                <td style={{ padding: '16px', color: '#fff', fontSize: '14px' }}>{log.confidence ? `${log.confidence.toFixed(2)}%` : 'N/A'}</td>
